      ...
      <div className='w-full bg-primary text-white'>
        <div className='container mx-auto flex items-center justify-between px-4 py-2 text-sm'>
          <div className='flex flex-col gap-1 leading-tight text-white'>
            <a href={`mailto:${CONTACT_INFO.email}`} className='inline-flex items-center gap-2 hover:opacity-90'>
              <Mail className='size-4' />
              {CONTACT_INFO.email}
            </a>
            <a href={`tel:${phoneHref}`} className='inline-flex items-center gap-2 hover:opacity-90'>
      ...
      <div className='bg-white shadow-sm'>
        ...
        <div className='container mx-auto flex h-16 items-center justify-between px-4 lg:h-20'>
          ...
          <div className='flex shrink-0 items-center overflow-visible'>
            <div className='flex w-[220px] items-center overflow-visible lg:w-[300px]'>
              <Image
                src={BRAND_ASSETS.logo}
                width={200}
                height={200}
                alt='Trúc Xinh'
                className='h-10 w-auto origin-left -translate-x-[100px] translate-y-[10px] scale-[6] object-contain lg:h-12 lg:-translate-x-[100px] lg:translate-y-[8px] lg:scale-[5]'
              />
            </div>
          </div>
          ...
          <nav className='hidden items-center gap-6 text-[15px] font-semibold text-primary lg:flex'>
            {NAV_ITEMS.map((item) => {
              if (!item.children?.length) {
                return (
                  <Link key={item.label} href={item.href} className='transition-colors hover:text-primary/70'>
                    {item.label}
                  </Link>
                )
              }
              ...
              return (
                <div key={item.label} className='group relative'>
                  <Link
                    href={item.href}
                    className='inline-flex items-center gap-1 transition-colors hover:text-primary/70'
                  >
                    {item.label}
